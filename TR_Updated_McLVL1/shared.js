// ─── PAYLAŞILAN ARAÇLAR — Minecraft Education Tutor Eğitimi ─────────────────
...
function activateDevMode() {
  ...
  saveProgress(p);
  showDevBadge(true);
  alert('🔓 Geliştirici Modu ETKİNLEŞTİRİLDİ — tüm modüller kilitsiz. Devre dışı bırakınca ilerleme sıfırlanır.');
}
function deactivateDevMode() {
  localStorage.removeItem('mc-devmode');
  saveProgress({});
  showDevBadge(false);
  alert('🔒 Geliştirici Modu DEVRE DIŞI — ilerleme silindi.');
}
...
function showDevBadge(on) {
  ...
  if (!b) {
    ...
    b.textContent = '🔓 GELİŞTİRİCİ MODU';
...
function injectRegionSwitcher() {
  ...
  const FLAGS = {
    ...
    tr: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 40" width="20" height="14" style="vertical-align:middle;border-radius:2px"><rect width="60" height="40" fill="#E30A17"/><circle cx="23" cy="20" r="10" fill="#fff"/><circle cx="26.5" cy="20" r="8" fill="#E30A17"/><polygon points="35,20 42,15 40,20 42,25" fill="#fff"/><polygon points="38,20 44,17 43,20 44,23" fill="#fff"/></svg>',
    id: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 40" width="20" height="14" style="vertical-align:middle;border-radius:2px"><rect width="60" height="20" fill="#CE1126"/><rect y="20" width="60" height="20" fill="#fff"/></svg>',
  };
  ...
  const REGIONS = [
    ...
    { key:"tr", label:"TR",  path:"../TR_Updated_McLVL1/Tr.html",    msg: null },
    { key:"id", label:"ID",  path:"../ID_Updated_McLVL1/Id.html",    msg: "Endonezce eğitime geçmek üzeresiniz.\n\nAnda akan beralih ke pelatihan Bahasa Indonesia." },
  ];
  ...
  REGIONS.forEach(r => {
    ...
    opt.className = 'region-opt' + (r.key === 'tr' ? ' active' : '');
  ...
  homeLink.innerHTML = '🌍 &nbsp;Bölgeyi değiştir';
  ...
  overlay.innerHTML = `
    <div id="region-modal">
      ...
      <h3 id="rm-title">Bölge Değiştir</h3>
      <p id="rm-body"></p>
      <div class="rm-btns">
        <button class="rm-no" id="rm-cancel">İptal</button>
        <button class="rm-yes" id="rm-confirm">Aç →</button>
      </div>
    </div>`;
  ...
  function showRegionModal(region) {
    document.getElementById('rm-title').textContent = 'Şuna geç: ' + region.label;
