// ─── ALAT BERSAMA — Pelatihan Tutor Minecraft Education ─────────────────
...
function activateDevMode() {
  ...
  saveProgress(p);
  showDevBadge(true);
  alert('🔓 Mode Dev AKTIF — semua modul dibuka. Kemajuan direset saat dinonaktifkan.');
}
function deactivateDevMode() {
  localStorage.removeItem('mc-devmode');
  saveProgress({});
  showDevBadge(false);
  alert('🔒 Mode Dev NONAKTIF — kemajuan dihapus.');
}
...
function goHome() { window.location.href = 'Id.html'; }
function goModule(n) { window.location.href = `IdModule${n}.html`; }
function goTest(n) { window.location.href = `IdModule${n}Test.html`; }
function goDST() { window.location.href = 'IdDST.html'; }
...
function injectRegionSwitcher() {
  ...
  REGIONS.forEach(r => {
    ...
    opt.className = 'region-opt' + (r.key === 'id' ? ' active' : '');
  ...
  homeLink.innerHTML = '🌍 &nbsp;Ganti wilayah';
  ...
  overlay.innerHTML = `
    <div id="region-modal">
      ...
      <h3 id="rm-title">Ganti Wilayah</h3>
      <p id="rm-body"></p>
      <div class="rm-btns">
        <button class="rm-no" id="rm-cancel">Batal</button>
        <button class="rm-yes" id="rm-confirm">Buka →</button>
      </div>
    </div>`;
  ...
  function showRegionModal(region) {
    document.getElementById('rm-title').textContent = 'Beralih ke ' + region.label;
