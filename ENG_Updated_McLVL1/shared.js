// ─── SHARED UTILITIES — Minecraft Education Tutor Training ─────────────────
...
function activateDevMode() {
  ...
  saveProgress(p);
  showDevBadge(true);
  alert('🔓 Dev Mode ENABLED — all modules unlocked. Progress reset on disable.');
}
function deactivateDevMode() {
  localStorage.removeItem('mc-devmode');
  saveProgress({});
  showDevBadge(false);
  alert('🔒 Dev Mode DISABLED — progress cleared.');
}
...
// ── Dev Mode Activation (Shift+D × 3 within 2s) ─────────────────────────────
(function() {
...
function injectRegionSwitcher() {
  ...
  // Inline SVG flags — Windows safe, no emoji
  const FLAGS = {
  ...
  const REGIONS = [
    { key:'en', label:'ENG', path:'../ENG_Updated_McLVL1/En.html',   msg: null },
    { key:'it', label:'IT',  path:'../IT_Updated_McLVL1/it.html',    msg: 'You are about to switch to the Italian training.\n\nStai per passare alla formazione italiana.' },
    { key:'es', label:'ES',  path:'../LATAM_Updated_McLVL1/es.html', msg: 'You are about to switch to the Spanish training.\n\nEstás a punto de cambiar a la formación en español.' },
    { key:'pl', label:'PL',  path:'../POL_Updated_McLVL1/pol.html',  msg: 'You are about to switch to the Polish training.\n\nZamierzasz przejść do szkolenia polskiego.' },
    { key:'pt', label:'PT',  path:'../PT_Updated_McLVL1/pt.html',    msg: 'You are about to switch to the Portuguese training.\n\nVocê está prestes a mudar para o treinamento em português.' },
  ];
  ...
  REGIONS.forEach(r => {
    ...
    opt.className = 'region-opt' + (r.key === 'en' ? ' active' : '');
  ...
  homeLink.innerHTML = '🌍 &nbsp;Change region';
  ...
  overlay.innerHTML = `
    <div id="region-modal">
      ...
      <h3 id="rm-title">Switch Region</h3>
      <p id="rm-body"></p>
      <div class="rm-btns">
        <button class="rm-no" id="rm-cancel">Cancel</button>
        <button class="rm-yes" id="rm-confirm">Open →</button>
      </div>
    </div>`;
  ...
  function showRegionModal(region) {
    document.getElementById('rm-title').textContent = 'Switch to ' + region.label;
