...
function injectRegionSwitcher() {
    ...
    const REGIONS = [
        { key:'en', label:'ENG', path:'../ENG_Updated_McLVL1/En.html',   msg: 'Você está prestes a mudar para o treinamento em inglês.\n\nYou are about to switch to the English training.' },
        { key:'it', label:'IT',  path:'../IT_Updated_McLVL1/it.html',    msg: 'Você está prestes a mudar para o treinamento em italiano.\n\nStai per passare alla formazione italiana.' },
        { key:'es', label:'ES',  path:'../LATAM_Updated_McLVL1/es.html', msg: 'Você está prestes a mudar para o treinamento em espanhol.\n\nEstás a punto de cambiar a la formación en español.' },
        { key:'pl', label:'PL',  path:'../POL_Updated_McLVL1/pol.html',  msg: 'Você está prestes a mudar para o treinamento em polonês.\n\nZamierzasz przejść do szkolenia polskiego.' },
        { key:'pt', label:'PT',  path:'../PT_Updated_McLVL1/pt.html',    msg: null },
    ];
    ...
    REGIONS.forEach(r => {
        ...
        opt.className = 'region-opt' + (r.key === 'pt' ? ' active' : '');
    ...
    homeLink.innerHTML = '🌍 &nbsp;Mudar região';
    ...
    overlay.innerHTML = `
        <div id="region-modal">
            ...
            <h3 id="rm-title">Mudar Região</h3>
            <p id="rm-body"></p>
            <div class="rm-btns">
                <button class="rm-no" id="rm-cancel">Cancelar</button>
                <button class="rm-yes" id="rm-confirm">Abrir →</button>
            </div>
        </div>`;
    ...
    function showRegionModal(region) {
        document.getElementById('rm-title').textContent = 'Mudar para ' + region.label;
