...
function injectRegionSwitcher() {
  ...
  const REGIONS = [
    { key:'en', label:'ENG', path:'../ENG_Updated_McLVL1/En.html',   msg: 'Estás a punto de cambiar a la formación en inglés.\n\nYou are about to switch to the English training.' },
    { key:'it', label:'IT',  path:'../IT_Updated_McLVL1/it.html',    msg: 'Estás a punto de cambiar a la formación en italiano.\n\nStai per passare alla formazione italiana.' },
    { key:'es', label:'ES',  path:'../LATAM_Updated_McLVL1/es.html', msg: null },
    { key:'pl', label:'PL',  path:'../POL_Updated_McLVL1/pol.html',  msg: 'Estás a punto de cambiar a la formación en polaco.\n\nZamierzasz przejść do szkolenia polskiego.' },
    { key:'pt', label:'PT',  path:'../PT_Updated_McLVL1/pt.html',    msg: 'Estás a punto de cambiar a la formación en portugués.\n\nVocê está prestes a mudar para o treinamento em português.' },
  ];
  ...
  REGIONS.forEach(r => {
    ...
    opt.className = 'region-opt' + (r.key === 'es' ? ' active' : '');
  ...
  homeLink.innerHTML = '🌍 &nbsp;Cambiar región';
  ...
  overlay.innerHTML = `
    <div id="region-modal">
      ...
      <h3 id="rm-title">Cambiar Región</h3>
      <p id="rm-body"></p>
      <div class="rm-btns">
  ...
  function showRegionModal(region) {
    document.getElementById('rm-title').textContent = 'Cambiar a ' + region.label;
