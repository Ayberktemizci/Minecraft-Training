...
function injectRegionSwitcher() {
    ...
    const REGIONS = [
        { key:'en', label:'ENG', path:'../ENG_Updated_McLVL1/En.html',   msg: 'Zamierzasz przejść do szkolenia w języku angielskim.\n\nYou are about to switch to the English training.' },
        { key:'it', label:'IT',  path:'../IT_Updated_McLVL1/it.html',    msg: 'Zamierzasz przejść do szkolenia w języku włoskim.\n\nStai per passare alla formazione italiana.' },
        { key:'es', label:'ES',  path:'../LATAM_Updated_McLVL1/es.html', msg: 'Zamierzasz przejść do szkolenia w języku hiszpańskim.\n\nEstás a punto de cambiar a la formación en español.' },
        { key:'pl', label:'PL',  path:'../POL_Updated_McLVL1/pol.html',  msg: null },
        { key:'pt', label:'PT',  path:'../PT_Updated_McLVL1/pt.html',    msg: 'Zamierzasz przejść do szkolenia w języku portugalskim.\n\nVocê está prestes a mudar para o treinamento em português.' },
    ];
    ...
    if (!document.getElementById('region-switcher-style')) {
        ...
        style.textContent = `
            ...
            .region-home{display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;font-size:12px;font-weight:600;color:var(--text-muted);text-decoration:none;transition:background .12px}
    ...
    REGIONS.forEach(r => {
        ...
        opt.className = 'region-opt' + (r.key === 'pl' ? ' active' : '');
    ...
    homeLink.innerHTML = '🌍 &nbsp;Zmień region';
    ...
    overlay.innerHTML = `
        <div id="region-modal">
            ...
            <h3 id="rm-title">Zmień Region</h3>
            <p id="rm-body"></p>
            <div class="rm-btns">
                <button class="rm-no" id="rm-cancel">Anuluj</button>
                <button class="rm-yes" id="rm-confirm">Otwórz →</button>
            </div>
        </div>`;
    ...
    function showRegionModal(region) {
        document.getElementById('rm-title').textContent = 'Przejdź do ' + region.label;
